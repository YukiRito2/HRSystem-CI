...
class Employee extends CI_Controller
{
    ...
    public function Save()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $this->form_validation->set_rules('contact', 'contact', 'trim|required|min_length[8]|max_length[15]|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($_FILES['image_url']['name']) {
                    ...
                    $config = array(
                        'file_name' => $new_file_name,
                        'upload_path' => "./assets/images/users",
                        'allowed_types' => "gif|jpg|png|jpeg|svg|pdf|docx",
                        'overwrite' => False,
                        'max_size' => "20240000", // Can be set to particular file size , here it is 2 MB(2048 Kb)
                        'max_height' => "80000",
                        'max_width' => "80000"
                    ...
                    if (!$this->upload->do_upload('image_url')) {
                        ...
                        // Verificar si $employee es un objeto y si tiene la propiedad em_image
                        if (is_object($employee) && property_exists($employee, 'em_image')) {
                            $checkimage = "./assets/images/users/" . $employee->em_image;
                            if (file_exists($checkimage)) {
                                unlink($checkimage);
                            }
                        } else {
                            // Manejar el caso en que $employee no sea un objeto o no tenga la propiedad em_image
                            echo "Error: No se pudo obtener la imagen del empleado.";
                            // Puedes registrar este error o mostrar un mensaje más amigable al usuario
                        }
                        $path = $this->upload->data();
                        $img_url = $path['file_name'];
                        $data = array();
    ...
    public function getEmIdByCode($em_code)
    {
        ...
        if ($result) {
            ...
            return null; // o un valor que indique que no se encontró el empleado
        }
    }
